...
```{r}
Y0 = extract_period( Y , seas , calib )
```
...
```{r}
X = read_models( gcm , rcm , run , scenario , zone , ipath )
```

```{r}
X0 = extract_period( X , seas , calib )
X1 = extract_period( X , seas , proj  )
```
...
Z0 = bcm$predict(X0)
Z1 = bcm$predict(X1)
Z0 = postprocessing( Z0 , X0 )
Z1 = postprocessing( Z1 , X1 )
```
...
```{r}
basic_bias( X0 , Y0 , lZ0 )
```

```{r,fig.height=6,fig.width=9,fig.dpi=600}
plot_density( X0 , Y0 , Z0 )
```
...
```{r}
delta_projection( X0 , X1 , lZ0 , lZ1 )
```

```{r,fig.height=6,fig.width=9,fig.dpi=600}
plot_projection_density( X0 , Y0 , X1 , lZ1 )
```
...
bcm$fit( Y0 , X0 , X1 )
Z  = bcm$predict(X1,X0)
Z0 = postprocessing( Z$Z0 , X0 )
Z1 = postprocessing( Z$Z1 , X1 )
```
...
```{r}
etpY0 = find_ETP( Y0 , zone = zone )
etpX0 = find_ETP( X0 , zone = zone )
etpX1 = find_ETP( X1 , zone = zone )

letpZ0 = list()
letpZ1 = list()
for( bc in names(lZ0) )
{
	letpZ0[[bc]] = find_ETP( lZ0[[bc]] , zone = zone )
	letpZ1[[bc]] = find_ETP( lZ1[[bc]] , zone = zone )
}
```

```{r}
basic_bias( etpX0 , etpY0 , letpZ0 )
```

```{r,fig.height=6,fig.width=6,fig.dpi=600}
plot_density_ETP( etpX0 , etpY0 , letpZ0 )
```
...
```{r}
rvY0TM = ROOPSD::rv_histogram$new()$fit(Y0[,1])
rvY0TN = ROOPSD::rv_histogram$new()$fit(Y0[,2])
Y0TMu  = rvY0TM$rvs(1000)
Y0TNu  = rvY0TN$rvs(1000)
```

```{r,fig.height=6,fig.width=6,fig.dpi=600}
par( mfrow = base::c(1,1) )
plot( Y0[,1] , Y0[,2] , type = "p" , col = "blue" , xlim = base::c(5,28) , ylim = base::c(5,25) , xlab = "tas" , ylab = "tasmin" )
points( Y0TMu , Y0TNu , col = "red" )
```
...
Z0 = lZ0$cdft
par( mfrow = base::c(1,3) )
i = "tas"
j = "sfcWind"
ylim = c(0,10)
xlim = c(0,30)
plot( Y0[,i] , Y0[,j] , type = "p" , col = "blue" , xlim = xlim , ylim = ylim , xlab = i , ylab = j )
points( X0[,i] , X0[,j] , col = grDevices::rgb(1,0,0,0.2) )

plot(   Y0[,i] , Y0[,j] , type = "p" , col = "blue" , xlim = xlim , ylim = ylim , xlab = i , ylab = j )
points( Z0[,i] , Z0[,j] , col = grDevices::rgb(0,1,0,0.5) )

plot(   X0[,i] , X0[,j] , type = "p" , col = "red" , xlim = xlim , ylim = ylim , xlab = i , ylab = j )
points( Z0[,i] , Z0[,j] , col = grDevices::rgb(0,1,0,0.5) )
```
...
```{r}
corr_bias( X0 , Y0 , lZ0 , relatif = FALSE )
```
...
```{r}
corr_bias( X0 , Y0 , lZ0 , relatif = TRUE )
```
...
par( mfrow = base::c(1,1) )
colorsZ = base::c( "orange" , "green" , "purple" )

plot( 1:n_day , nb_cluster_0( Y0[,6] , n_day ) , type = "l" , col = "blue" , xlab = "Taille cluster" , ylab = "Nombre de cluster" , ylim = c(0,400) )
for( i in 1:length(lZ0) )
{
	Z0 = lZ0[[i]]
	lines( 1:n_day , nb_cluster_0( Z0[,6] , n_day ) , type = "l" , col = colorsZ[i] )
}
```
...
```{r}
for( m in base::c("06","07","08") )
{
	X0m = subset( X0 , format.Date( rownames(X0) , "%m" ) == m )
	Y0m = subset( Y0 , format.Date( rownames(Y0) , "%m" ) == m )
	
	lZ0m = list()
	for( bc in names(lZ0) )
	{
		lZ0m[[bc]] = subset( lZ0[[bc]] , format.Date( rownames(lZ0[[bc]]) , "%m" ) == m )
	}
	
	cat( "//====================\\\\" , end = "\n" )
	cat( paste0( "|| Month " , m , "           ||" ) , end = "\n" )
	cat( "\\\\====================//" , end = "\n" )
	basic_bias( X0m , Y0m , lZ0m )
}
```
...
```{r}
bcm = SBCK::AR2D2$new( col_cond = 1 , lag_search = 6 , lag_keep = 3 )
bcm$fit( Y0 , X0 , X1 )
Z  = bcm$predict(X1,X0)
Z0 = postprocessing( Z$Z0 , X0 )
Z1 = postprocessing( Z$Z1 , X1 )
...
par( mfrow = base::c(1,1) )
colorsZ = base::c( "orange" , "green" , "purple" , "black" )
